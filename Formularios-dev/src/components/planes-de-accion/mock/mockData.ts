...
export const MOCK_TAREAS: TareaObservacion[] = [
  {
    ...
    descripcionObservacion:
      "Pernos de anclaje sin tensión adecuada en sector norte",
    accionPropuesta:
      "Reajustar todos los pernos según protocolo de fortificación",
  ...
  },
  {
    ...
    descripcionObservacion:
      "Herramientas sin guardas de seguridad, aceite derramado en piso",
    accionPropuesta:
      "Implementar guardas en esmeriles, limpiar derrames, capacitar personal",
    ...
    tareas: [
      {
        ...
        descripcionObservacion:
          "Cables expuestos sin aislamiento en banco de trabajo",
      ...
      },
      {
        ...
        familiaPeligro: "Orden y Limpieza",
        descripcionObservacion:
          "Estanterías sobrecargadas sin señalización de peso máximo",
        accionPropuesta:
          "Redistribuir carga y señalizar capacidad de estanterías",
    ...
    tareas: [
      {
        ...
        descripcionObservacion:
          "Hojas de seguridad (HDS) desactualizadas y no visibles",
        accionPropuesta:
          "Actualizar HDS y ubicar en lugar visible cerca de productos",
      ...
      },
      {
        ...
        descripcionObservacion:
          "Grúa horquilla sin certificación vigente de mantención",
...
export const calcularDiasRetraso = (
  ...
  if (estado === "cerrado" && fechaCumplimientoEfectiva) {
    const diff =
      new Date(fechaCumplimientoEfectiva).getTime() -
      new Date(fechaCumplimientoAcordada).getTime();
    return Math.max(0, Math.floor(diff / (1000 * 60 * 60 * 24)));
  }

  if (estado !== "cerrado") {
    const diff =
      new Date().getTime() - new Date(fechaCumplimientoAcordada).getTime();
    return Math.max(0, Math.floor(diff / (1000 * 60 * 60 * 24)));
  }
