...
export function OutOfServiceSection({
  ...
  // Filtrado condicional según elementos activos del arnés seleccionados en ROOT
  let fieldsToRender = sectionConfig.fields;
  if (selectedRootItems && selectedRootItems.length > 0) {
    fieldsToRender = sectionConfig.fields.filter(field => {
      const name = field.fieldName.toLowerCase();
      if (name.includes("arnes")) {
        return selectedRootItems.some(item => item.startsWith("ARNÉS DE CUERPO ENTERO"));
      }
      if (name.includes("autoretractil")) {
        return selectedRootItems.some(item => item.startsWith("AUTORETRACTIL PERSONAL"));
      }
      if (name.includes("conectoranclaje")) {
        return selectedRootItems.some(item => item.startsWith("CONECTORES DE ANCLAJE"));
      }
      if (name.includes("conector")) {
        // Asegurarse de que no sea "conectoranclaje" (ya manejado arriba)
        return selectedRootItems.some(item => item.startsWith("CONECTORES DE TEJIDO TRENZADO"));
      }
      return true; // Mostrar siempre por defecto si no coincide con ninguno
    });
  }
