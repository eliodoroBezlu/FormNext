...
export function DynamicSectionSelector({
  ...
  const isRootSelector = config.sectionTitle === "ROOT";

  // Encontrar la sección padre si no es root selector
  ...
  const parentSection = isRootSelector ? null : findSection(sections, config.sectionTitle);

  if (!isRootSelector && (!parentSection || !parentSection.subsections || parentSection.subsections.length === 0)) {
    return null;
  }
  ...
  return (
    <Paper elevation={1} sx={{ p: 3, mb: 3 }}>
      ...
      <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1.5 }}>
        {(isRootSelector ? sections : (parentSection?.subsections || [])).map((item) => {
            ...
            return (
              <Box key={idx} sx={{ ml: 2, mb: 2 }}>
                <DynamicSectionSelector
                  sections={isRootSelector ? sections : (parentSection?.subsections || [])}
                  config={nestedConfig}
