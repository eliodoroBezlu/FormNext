...
export function ScaffoldInspectionForm({
  ...
  const getInfoAlert = () => {
    if (initialData?.status === InspectionStatus.PENDING_APPROVAL) {
      return (
        <Alert severity="warning">
          ⏳ <strong>Pendiente de aprobación:</strong> Esta inspección está
          esperando la aprobación de un supervisor para habilitar el andamio.
        </Alert>
      );
    }
    ...
    if (isInProgress) {
      return (
        <Alert severity="info">
          📋 <strong>Modo continuo:</strong> Este andamio está en uso. Solo
          puede agregar inspecciones rutinarias. Los datos iniciales están
          bloqueados.
        </Alert>
      );
    }
