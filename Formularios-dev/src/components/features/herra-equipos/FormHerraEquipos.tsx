...
function LlenarFormulariosContent() {
  ...
  const refreshCounts = async () => {
    ...
    try {
      ...
      console.error(err);
      // Falla silenciosa al refrescar contadores en segundo plano
    }
  };
  ...
  useEffect(() => {
    ...
    setVisitedTabs((prev) => (prev[targetTab] ? prev : { ...prev, [targetTab]: true }));
  ...
  const handleTabChange = (event: React.SyntheticEvent, newValue: number) => {
    ...
    setVisitedTabs((prev) => (prev[newValue] ? prev : { ...prev, [newValue]: true }));
    ...
    return (
      <Box p={3}>
        <Alert severity="error">
          No se pudo obtener información del usuario. Por favor, inicia sesión
          nuevamente.
        </Alert>
      </Box>
    );
  ...
  return (
    <Box p={3}>
      ...
      <TabPanel value={selectedTab} index={0}>
        {templates.length === 0 ? (
          <Alert severity="info">
            No hay templates disponibles. Primero debes crear templates en la
                  ...
                  >
                    ...
                    <CardContent sx={{ flexGrow: 1 }}>
                      ...
                      <Box display="flex" gap={1} flexWrap="wrap">
                        <Chip
                          label={
                            template.type === "interna" ? "Interna" : "Externa"
                          }
                          size="small"
                          color={
        ...
        {visitedTabs[1] && (
          <InProgressInspectionsList
            ...
            onActionSuccess={(msg: string) =>
              handleOperationSuccess(msg, "¡Inspección Actualizada!")
            }
          />
        )}
      </TabPanel>
