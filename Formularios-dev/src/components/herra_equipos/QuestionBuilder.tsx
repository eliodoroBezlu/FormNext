...
import { createTemplateHerraEquipo, deleteTemplateHerraEquipo, getTemplatesHerraEquipos, updateTemplateHerraEquipo } from "@/lib/actions/template-herra-equipos";
...
const QuestionBuilder: React.FC<QuestionBuilderProps> = ({
  ...
  const handleTypeChange = (newType: ResponseType) => {
    ...
    if (newType === "si_no_na") {
      ...
      setValue(
        optionsKey as keyof FormBuilderDataHerraEquipos,
        DEFAULT_SI_NO_NA_OPTIONS as never
      );
    }
  };
  ...
  const handleSaveImage = () => {
    ...
    setValue(
      imageKey as keyof FormBuilderDataHerraEquipos,
      { url: finalUrl, caption: imageCaption.trim() } as never
    );
  ...
  return (
    <Paper elevation={1} sx={{ p: 2, mb: 2, backgroundColor: "#fafafa" }}>
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <FormControl fullWidth size="small" disabled={disabled}>
                ...
                <Select
                  value={responseType}
                  onChange={(e) =>
                    handleTypeChange(e.target.value as ResponseType)
                  }
      ...
      >
        <DialogTitle>
          {question.image ? "Editar Imagen" : "Agregar Imagen"}
        </DialogTitle>
        <DialogContent>
          <Box sx={{ pt: 2 }}>
            {imageError && (
        ...
        <DialogActions>
          <MuiButton onClick={handleCancelDialog}>
            Cancelar
          </MuiButton>
          <MuiButton
...
const ImageManager: React.FC<ImageManagerProps> = ({
  ...
  return (
    <Box>
      ...
      >
        <DialogTitle>
          {editingIndex !== null ? "Editar Imagen" : "Agregar Imagen"}
        </DialogTitle>
        <DialogContent>
          <Box sx={{ pt: 2 }}>
            {imageError && (
...
const TemplateManagementApp: React.FC = () => {
  ...
  >("list");
  const [selectedTemplate, setSelectedTemplate] = useState<FormTemplateHerraEquipos | null>(
    null
  );
    ...
    if (result?.success) {
      setSuccessMessage(
        `Template "${template.name}" ${
          currentView === "create" ? "creado" : "actualizado"
        } exitosamente`
      );
      setCurrentView("list");
      setSelectedTemplate(null);
