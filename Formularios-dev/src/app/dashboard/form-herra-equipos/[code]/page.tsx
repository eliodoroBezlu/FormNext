...
import { obtenerDisponibilidadEquipos } from "@/lib/actions/equipment-tracking";
import { Trabajador } from "@/types/trabajador";

/**
 * Decide si una plantilla usa la UI especializada (stepper de 5 pasos, firmas,
 * observaciones generales, aprobación) o el `FormFiller` genérico.
 *
 * Se deriva del registro de configuraciones —que ya es la fuente de verdad de
 * `UnifiedFormRouter`, el cual enruta por `formType`— en vez de una lista
 * hardcodeada de códigos. Con la lista, una plantilla nueva con config
 * completa caía igual al `FormFiller` y perdía silenciosamente stepper,
 * firmas y observaciones; ahora basta con darle su config.
 */
...
export default function FormularioDinamicoPage() {
  ...
  const code = decodeURIComponent(
    (params.code || params.templateCode) as string,
  ).toUpperCase();

  const [template, setTemplate] = useState<FormTemplateHerraEquipos | null>(
    null,
  );
  ...
  const [verifiedEquipmentId, setVerifiedEquipmentId] = useState<string | null>(
    null,
  );
  const [verifiedTemplateCode, setVerifiedTemplateCode] = useState<
    string | null
  >(null);

  const [duplicateData, setDuplicateData] =
    useState<FormDataHerraEquipos | null>(null);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        console.log(
          `🔍 [PAGE] Cargando - Code: ${code}, InspectionId: ${inspectionId || "nuevo"}`,
        );
        ...
        if (inspectionId) {
          ...
          if (!inspectionResult.success) {
            throw new Error(
              inspectionResult.error || "Error al cargar inspección",
            );
          }
        ...
        // ============================================
          ...
          try {
            ...
            if (Array.isArray(areaData)) {
              const mapped = (
                areaData as (string | { nombre?: string; name?: string })[]
              )
                .map((a) =>
                  typeof a === "string" ? a : a.nombre || a.name || "",
                )
                .filter(Boolean);
          ...
          if (TEMPLATE_EQUIPMENT_MAP[code]) {
            try {
              ...
              relatedEquipos = allEquipos.filter((e) =>
                allowedTypes.includes(e.tipo_equipo),
              );

              // Si el template tiene frecuencia de inspección activa, ocultar
              // los códigos que todavía no cumplen su próxima fecha de
              // inspección. Si no tiene frecuencia configurada, el backend
              // devuelve todos como disponibles — sin cambio de comportamiento.
              try {
                const disponibilidad = await obtenerDisponibilidadEquipos(code);
                if (disponibilidad.length > 0) {
                  ...
                  relatedEquipos = relatedEquipos.filter((e) =>
                    codigosDisponibles.has(e.codigo),
                  );
                }
              } catch (err) {
                console.error(
                  "Error al obtener disponibilidad de equipos:",
                  err,
                );
              }
          ...
          if (requiresVerification) {
            ...
            const preverifiedEquipmentId = sessionStorage.getItem(
              "preverified_equipment_id",
            );
            const preverifiedFromForm = sessionStorage.getItem(
              "preverified_from_form",
            );
            const verificationTimestamp = sessionStorage.getItem(
            ...
            if (preverifiedEquipmentId && isVerificationValid) {
              console.log(
                `✅ [PAGE] TAG pre-verificado: ${preverifiedEquipmentId}`,
              );
            ...
            if (relatedEquipos.length === 0) {
              ...
              console.log(
                "⚠️ No hay equipos de este tipo en base de datos. Saltando selección...",
              );
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              ...
              foundTemplate.verificationFields.forEach((f) => {
                if (isAreaField(f.label) && userTrabajador?.area) {
                  ...
                  isSuperintendenciaField(f.label) &&
                  userTrabajador?.superintendencia
                ) {
                  prefilledVerification[f.label] =
                    userTrabajador.superintendencia;
                }
              });
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ...
            console.log(
              `✅ [PAGE] Form ${code} sin registro de equipos. Pre-llenando con datos del usuario y saltando a Step 2.`,
            );
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const prefilledVerification: Record<string, any> = {};
            foundTemplate.verificationFields.forEach((f) => {
          ...
          if (storedDuplicate) {
            try {
              ...
              setSnackbar({
                open: true,
                message:
                  "📋 Formulario pre-llenado con datos de inspección duplicada",
                severity: "info",
              });
            } catch (error) {
  ...
  const handleSelectionContinue = () => {
    ...
    if (template) {
      template.verificationFields.forEach((field) => {
        const value = resolveAutofillValue(
          field.label,
          selectedArea,
          selectedCode,
          selectedEquip,
  ...
  const getFilteredEquipos = () => {
    ...
    const normSelected = selectedArea
      .toUpperCase()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .trim();
    return equipos.filter((e) => {
      ...
      const normArea = areaNombre
        .toUpperCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .trim();
      return (
        normArea === normSelected ||
        normArea.includes(normSelected) ||
        normSelected.includes(normArea)
      );
    });
  };
  ...
  const handleSaveDraft = async (data: FormDataHerraEquipos) => {
    ...
    try {
      localStorage.setItem(
        `draft_${code}`,
        JSON.stringify(formDataWithEquipment),
      );
      ...
      if (result.success) {
        ...
        if (result.data?._id) {
          router.push(
            `/dashboard/form-herra-equipos/${code}/${result.data._id}?step=5`,
          );
        }
        return result.data;
      } else {
      ...
      setSnackbar({
        open: true,
        message:
          error instanceof Error ? error.message : "Error al guardar borrador",
        severity: "error",
      });
    } finally {
      setSaving(false);
  ...
  const handleFinalize = async (data: FormDataHerraEquipos) => {
    ...
    try {
      ...
      if (inspectionId) {
        console.log(
          `🔄 [PAGE] Finalizando inspección existente: ${inspectionId}`,
        );
          ...
          <CardContent>
            ...
            <Grid container spacing={3} sx={{ mb: 4 }}>
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <FormControl
                  fullWidth
                  disabled={!selectedArea || filteredOptions.length === 0}
                >
                  <InputLabel id="select-code-label">
                    TAG / Código de Herramienta
                  </InputLabel>
                  <Select
                  ...
                  >
                    ...
                    {filteredOptions.map((opt) => (
                      <MenuItem key={opt._id} value={opt.codigo}>
                        {opt.codigo}{" "}
                        {opt.descripcion ? `(${opt.descripcion})` : ""}
                      </MenuItem>
                    ))}
                  </Select>
                </FormControl>
  ...
  const initialFormData =
    existingInspection || duplicateData || prefilledData || undefined;
    ...
    <Box>
      ...
      {existingInspection?.status === "in_progress" && (
        <Alert severity="warning" sx={{ m: 2 }}>
          ...
          {existingInspection.verification?.TAG &&
            ` - Equipo: ${existingInspection.verification.TAG}`}
