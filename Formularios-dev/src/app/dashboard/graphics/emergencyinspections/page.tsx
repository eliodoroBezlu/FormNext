...
const DashboardEmergencyInspections: React.FC = () => {
  ...
  const coberturaInspecciones = useMemo(() => {
    let tagsFiltrados = tags.filter((t) => t.activo === true);
    if (filtroSuperintendencia) {
      tagsFiltrados = tagsFiltrados.filter(
        (t) => t.superintendencia === filtroSuperintendencia,
      );
    }
    if (filtroAreas.length > 0) {
      tagsFiltrados = tagsFiltrados.filter((t) => filtroAreas.includes(t.area));
    }
    if (filtroTags.length > 0) {
      tagsFiltrados = tagsFiltrados.filter((t) => filtroTags.includes(t.tag));
    }
    const totalTagsCount = tagsFiltrados.length;
    return totalTagsCount > 0
      ? Math.min(
          Math.round((inspeccionesFiltradas.length / totalTagsCount) * 100),
          100,
        )
      : 0;
  }, [
  ...
  const cumplimientoExtintores = useMemo(() => {
    let extintoresFiltrados = extintores.filter((e) => e.activo === true);
    if (filtroSuperintendencia) {
    ...
    const totalActivos = extintoresFiltrados.length;

    const totalInspeccionados = inspeccionesFiltradas.reduce((sum, ins) => {
  ...
  const alertasCriticas = useMemo(
    () => estadisticasGlobales.filter((area) => area.estado === "CRITICO"),
