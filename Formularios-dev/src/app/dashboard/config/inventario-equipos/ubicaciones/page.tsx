...
const initialFormData: UbicacionForm = {
  nombre: "",
  activo: true,
}
...
export default function GestionUbicaciones() {
  ...
  const [editingItem, setEditingItem] = useState<UbicacionBackend | null>(null)
  const [formData, setFormData] = useState<UbicacionForm>(initialFormData)
  ...
  const handleOpenEditModal = (item: UbicacionBackend) => {
    setEditingItem(item)
    setFormData({
      nombre: item.nombre,
      activo: item.activo,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.nombre.trim()) {
      showNotification("El nombre de la ubicación es requerido", "warning")
      return
    }

    try {
      if (editingItem) {
        await actualizarUbicacion(editingItem._id, formData)
        showNotification("Ubicación actualizada con éxito", "success")
      } else {
        await crearUbicacion(formData)
        showNotification("Ubicación creada con éxito", "success")
      }
      handleCloseModal()
      cargarDatos()
    } catch (err) {
      showNotification(err instanceof Error ? err.message : "Error al guardar ubicación", "error")
    }
  }
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("¿Está seguro de que desea eliminar esta ubicación?")) {
      try {
        ...
        showNotification(err instanceof Error ? err.message : "Error al eliminar ubicación", "error")
      }
    }
  }
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      <Dialog open={openModal} onClose={handleCloseModal} maxWidth="sm" fullWidth>
        <form onSubmit={handleSubmit}>
          <DialogTitle>
            {editingItem ? "Editar Ubicación" : "Agregar Nueva Ubicación"}
          </DialogTitle>
